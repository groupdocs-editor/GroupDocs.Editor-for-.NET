                ...
                using (EditableDocument beforeEdit = editor.Edit(editOptions))
                {
                    ...
                    )
                    {
                        ...
                        foreach (WordProcessingFormats oneFormat in Formats.WordProcessingFormats.All)
                        {
                            ...
                            Options.WordProcessingSaveOptions saveOptions = new WordProcessingSaveOptions(oneFormat);

                            //8.2. Prepare save path
                            string savePath = Path.Combine(FilesHelper.OutputFolder, "MultipleOutFormats." + saveOptions.OutputFormat.Extension);
