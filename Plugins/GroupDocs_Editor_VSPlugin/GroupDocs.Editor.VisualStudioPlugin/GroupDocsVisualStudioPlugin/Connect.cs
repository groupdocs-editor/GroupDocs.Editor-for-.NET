...
using GroupDocsEditorVisualStudioPlugin.GUI;
using GroupDocsEditorVisualStudioPlugin.Properties;

namespace GroupDocsEditorVisualStudioPlugin
{
    public class Connect : Extensibility.IDTExtensibility2, IDTCommandTarget
    {
        ...
        private const string GROUPDOCS_MENU_TOOLTIP = "Download and Run GroupDocs.Editor for .NET API Examples";

        private const string Editor_COMMAND_NAME = "GroupDocsEditorVSPlugin";
        private const string Editor_COMMAND_CAPTION = "New GroupDocs.Editor Example Project";
        private const string Editor_COMMAND_TOOLTIP = "Download and Run GroupDocs.Editor for .NET API Examples";
            ...
            try
            {
                ...
                for (int iloop = 1; iloop <= toolsPopup.CommandBar.Controls.Count; iloop++)
                {
                    if (toolsPopup.CommandBar.Controls[iloop].Caption == GROUPDOCS_MENU_CAPTION)
                    {
                        ...
                        {
                            if (cmdbtn.Caption == Editor_COMMAND_CAPTION)
                            {
                                oBtn = cmdbtn;
                            }
                        }
                ...
                if (oBtn == null)
                {
                    ...
                    Command myCommand = commands.AddNamedCommand2(addInInstance, Editor_COMMAND_NAME,
                        Editor_COMMAND_CAPTION, Editor_COMMAND_TOOLTIP, false, Resources.pnglogosmall, ref contextGUIDS,
           ...
           ref object varOut, ref bool handled)
        {
            try
            {
                ...
                {
                    if (cmdName == addInInstance.ProgID + "." + Editor_COMMAND_NAME)
                    {
